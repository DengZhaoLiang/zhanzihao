<template>
    <div class='c-good-list-item'>
        <div>
            <el-checkbox-group @change='handleCheckedGoodsChange' v-model='checkedGoods'>
                <div :key='key' class='c-row-container' v-for='(item,key) in getcarts'>
                    <el-row class='c-row'>
                        <el-col :span='isShowAccount ? 1 : 2' v-if='showCheckBox'>
                        ...
                        <el-col :span='4'>
                            <c-input-number :index='key' :num='item.purchaseNum' @change='onInputChange'
                                            v-if='isShowAccount' />
                            <span class='account-num' v-else>{{ item.purchaseNum }}</span>
                        </el-col>
                        <el-col :span='isShowAccount ? 5 : 6' class='yx-center'>
                            <c-money :money='item.product.price * (item.purchaseNum)'
                                     size='sm'></c-money>
                        </el-col>
                        <el-col :span='2' v-if='isShowAccount'>
                            <div @click='showDelModal(item.id)' class='c-del-good'>
                            </div>
                        </el-col>
                    </el-row>
                </div>
        ...
        <!-- 商品汇总 -->
        <div class='checkout'>
            <div class='c-total-container' v-if='isShowTotal'>
                <p>普通商品&nbsp;<span>{{ checkedGoods.length }}&nbsp;</span>件</p>
                <div class='total-container xy-center'>
                    <img @click="loadMoreImg('left')" class='arrow-left'
                         src='../../../../public/images/cart/left.png'>
                    <div class='img-container xy-center'>
                        <img :key='key'
                             :src='item'
                             v-for='(item,key) in getImgList'>
                    </div>
                    <img @click="loadMoreImg('right')" class='arrow-right'
